        ...
        .open(serial_console_path)
    {
        Ok(mut serial_console) => {
            if let Err(e) = write_all_with_timeout(
                ...
                write_warning(format!("write_serial_console_log::Failed to write to serial console: {e}"));
            }
        }
        Err(e) => {
            write_warning(format!("write_serial_console_log::Failed to open serial console: {e}"));
        }
    }
}
