                        ...
                        if (alternativeSizes.Count > 0)
                        {
                            ...
                            foreach (var altSize in alternativeSizes)
                            {
                                try
                                {
                                    ...
                                    vmr = await _vmBuilder.Build(this.EnableProxyAgentForNewVM, altSize, cancellationToken);
                                    ConsoleLog($"VM Create succeed with alternative VM size: {altSize}");
                                    sw.Stop();
                                    _junitTestResultBuilder.AddSuccessTestResult(_testScenarioSetting.testScenarioName, vmCreateTestName, $"VM Create succeed with alternative VM size: {altSize}", "", sw.ElapsedMilliseconds);
                                    retrySucceeded = true;
