...
set Configuration=%1
set CPU_Architecture=%2
set CleanBuild=%3
set ContinueAtConvertBpfToNative=%4

if "%Configuration%"=="" (SET Configuration=debug)
if "%CPU_Architecture%"=="arm64" (SET BuildTarget=aarch64-pc-windows-msvc) else (SET BuildTarget=x86_64-pc-windows-msvc)
echo Configuration=%Configuration%
echo BuildTarget=%BuildTarget%
echo out_path=%out_path%
SET out_dir=%out_path%\%BuildTarget%\%Configuration%
...
call rustup target install %BuildTarget%
...
echo call cargo build %release_flag% --target %BuildTarget% --manifest-path %cargo_toml% --target-dir %out_path%
call cargo build %release_flag% --target %BuildTarget% --manifest-path %cargo_toml% --target-dir %out_path%
if  %ERRORLEVEL% NEQ 0 (
...
if "%CPU_Architecture%"=="arm64" (
    echo ======= skip running proxy_agent_shared tests on arm64
) else (
    echo ======= run rust proxy_agent_shared tests
    echo call cargo test --all-features  %release_flag% --target %BuildTarget% --manifest-path %cargo_toml% --target-dir %out_path% -- --test-threads=1
    call cargo test --all-features  %release_flag% --target %BuildTarget% --manifest-path %cargo_toml% --target-dir %out_path% -- --test-threads=1
    if  %ERRORLEVEL% NEQ 0 (
...
if "%CPU_Architecture%"=="arm64" (
    echo ======= skip running proxy_agent tests on arm64
) else (
...
if "%CPU_Architecture%"=="arm64" (
    echo ======= skip running proxy_agent_extension tests on arm64
) else (
...
if "%CPU_Architecture%"=="arm64" (
    echo ======= skip restore and build restore e2e test project on arm64
) else (    
...
call powershell.exe Compress-Archive -Path "%out_package_dir%" -DestinationPath "%out_dir%"\build-%Configuration%-windows-%CPU_Architecture%.zip" -Force